...
import { generateBoard, renderAttack } from "../src/modules/domManager";

jest.mock('../src/modules/domManager.js', () => ({
   generateBoard: jest.fn(),
   renderAttack: jest.fn()
  }))

describe("Starting the game", () => {
  generateBoard.mockImplementation(() => "test")
  const game = new Game()
  ...
  describe("Does the AI?", () => {
    ...
    test("correctly adjust aim?", () => {
      expect(ai.makeAttack([5,5])).not.toMatchObject({loc: [5,5]})
    })
  ...
  describe("Does the game?", () => {
    test("correctly switch turns?", () => {
      renderAttack.mockImplementation(() => "test")
      game.takeTurn()
